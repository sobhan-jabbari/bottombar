...
public class BottomBar extends LinearLayout implements View.OnClickListener, View.OnLongClickListener {
  ...
  private void populateAttributes(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    try {
      ...
      activeIconScale = ta.getFloat(R.styleable.BottomBar_bb_activeIconScale, 1.24f);
      inActiveIconScale = ta.getFloat(R.styleable.BottomBar_bb_inActiveIconScale, 1f);
      activeTitleScale = ta.getFloat(R.styleable.BottomBar_bb_activeTitleScale, 1f);
      inActiveTitleScale = ta.getFloat(R.styleable.BottomBar_bb_inActiveTitleScale, 0.86f);
      activeIconTranslateY = ta.getDimension(R.styleable.BottomBar_bb_activeIconTranslateY, 0f);
      inActiveIconTranslateY = ta.getDimension(R.styleable.BottomBar_bb_inActiveIconTranslateY, 0f);
      ...
      tintIcon = ta.getBoolean(R.styleable.BottomBar_bb_tintIcon, true);
      tintMode = ta.getInt(R.styleable.BottomBar_bb_tintMode, 0);
  ...
  private void determineInitialBackgroundColor() {
    ...
    } else if (userDefinedBackground != null) {
  ...
  /**
   * Set the item for the BottomBar
   */
  public void setItems(@NonNull List<BottomBarTab> bottomBarItems) {
    BottomBarTab.Config defaultTabConfig = getTabConfig();
  ...
  private BottomBarTab.Config getTabConfig() {
    return new BottomBarTab.Config.Builder()
      ...
      .inActiveIconScale(inActiveIconScale)
      .activeTitleScale(activeTitleScale)
      .inActiveTitleScale(inActiveTitleScale)
      .activeIconTranslate(activeIconTranslateY)
      .inActiveIconTranslate(inActiveIconTranslateY)
  ...
  private void updateItems(final List<BottomBarTab> bottomBarItems) {
    ...
    for (BottomBarTab bottomBarTab : bottomBarItems) {
      ...
      if (isIconsOnlyMode()) {
        bottomBarTab.setTitleless(true);
      }
  ...
  private void resizeTabsToCorrectSizes(BottomBarTab[] tabsToAdd) {
    if (tabsToAdd == null || tabsToAdd.length == 0) return;
    ...
    int proposedItemWidth = Math.min(
      MiscUtils.dpToPixel(getContext(), (float) viewWidth / tabsToAdd.length),
      maxFixedItemWidth
    );
  ...
  public int getCurrentTabId() {
    return getCurrentTab().getItemId();
  }
  ...
  public BottomBarTab getTabWithId(int tabId) {
    ...
    for (int i = 0; i < tabCount; i++) {
      child = tabContainer.getChildAt(i);
      if (child instanceof BottomBarTab) {
        tab = (BottomBarTab) child;
        if (tab.getItemId() == tabId) {
          return tab;
        }
      }
    }
  ...
  public void setTintMode(int tintMode) {
    ...
    batchPropertyApplier.applyToAllTabs(tab -> tab.setTintMode(tintMode));
  }
  ...
  /**
   * Set scale used for selected BottomBarTabs.
   */
  public void setSelectedIconScale(float scale) {
    activeIconScale = scale;

    batchPropertyApplier.applyToAllTabs(tab -> tab.setActiveIconScale(activeIconScale));
  }

  /**
   * Set scale used for selected BottomBarTabs.
   */
  public void setInActiveTitleScale(float scale) {
    inActiveIconScale = scale;

    batchPropertyApplier.applyToAllTabs(tab -> tab.setInActiveTitleScale(inActiveIconScale));
  }

  /**
   * Set scale used for selected BottomBarTabs.
   */
  public void setSelectedTitleScale(float scale) {
    activeTitleScale = scale;

    batchPropertyApplier.applyToAllTabs(tab -> tab.setActiveTitleScale(activeTitleScale));
  }
  ...
  private void handleClick(BottomBarTab newTab) {
    if (newTab.isSpace()) return;
    BottomBarTab oldTab = getCurrentTab();

    if (tabSelectionInterceptor != null
      && tabSelectionInterceptor.shouldInterceptTabSelection(oldTab.getItemId(), newTab.getItemId())) {
      return;
    }
  ...
  private void updateSelectedTab(int newPosition) {
    int newTabId = getTabAtPosition(newPosition).getItemId();
